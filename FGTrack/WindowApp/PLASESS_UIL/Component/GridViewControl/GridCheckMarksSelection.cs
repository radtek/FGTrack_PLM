...
namespace HTN.BITS.UIL.PLASESS.Component.GridViewControl
{
    public class GridCheckMarksSelection
    {
        ...
        public GridCheckMarksSelection(GridView view, string fieldName, string filterString)
            : this()
        {
            View = view;
        ...
        public void SelectAll()
        {
            ...
            //    _view.ActiveFilterString = string.Format("[{0}] {1}", this.fieldName, this.activeFilterString);
            try
            {
            ...
            try
            {
                ...
                if (info.Column == column)
                {
                    ...
                    if (info.InRowCell)
                    {
                        if (!string.IsNullOrEmpty(this.activeFilterString))
                        {
                            string colVal = _view.GetRowCellDisplayText(info.RowHandle, this.fieldName);

                            switch (this.operater)
                            {
                                case "IS":

                                    switch (this.activeFilterString)
                                    {
                                        case "NULL":
                                    ...
                                    break;
                                default:
                                    if (!this.activeFilterString.Contains(colVal))
                                    {
                                        InvertRowSelection(info.RowHandle);
                                    }
                                    break;
