using System;
using QRCodeDecoder = HTN.BITS.QRCodeLib.QRCodeDecoder;
using HTN.BITS.QRCodeLib.Reader;
using AlignmentPatternNotFoundException = HTN.BITS.QRCodeLib.ExceptionHandler.AlignmentPatternNotFoundException;
using InvalidVersionException = HTN.BITS.QRCodeLib.ExceptionHandler.InvalidVersionException;
using HTN.BITS.QRCodeLib.Geom;
using HTN.BITS.QRCodeLib.Util;

namespace HTN.BITS.QRCodeLib.Reader.Pattern
{
    public class AlignmentPattern
    {
        ...
        internal static Point[][] getCenter(bool[][] image, FinderPattern finderPattern, Point[][] logicalCenters)
        {
            ...
            canvas.drawCross(centers[0][0], HTN.BITS.QRCodeLib.Util.Color_Fields.BLUE);

            axis.Origin = finderPattern.getCenter(FinderPattern.UR);
            centers[sqrtCenters - 1][0] = axis.translate(-3, 3);
            canvas.drawCross(centers[sqrtCenters - 1][0], HTN.BITS.QRCodeLib.Util.Color_Fields.BLUE);

            axis.Origin = finderPattern.getCenter(FinderPattern.DL);
            centers[0][sqrtCenters - 1] = axis.translate(3, -3);
            canvas.drawCross(centers[0][sqrtCenters - 1], HTN.BITS.QRCodeLib.Util.Color_Fields.BLUE);
            ...
            for (int y = 0; y < sqrtCenters; y++)
            {
                for (int x = 0; x < sqrtCenters; x++)
                {
                    ...
                    if (y == 0)
                    {
                        ...
                        canvas.drawCross(centers[x][y], HTN.BITS.QRCodeLib.Util.Color_Fields.RED);
                    }
                    else if (x == 0)
                    {
                    ...
                    canvas.drawCross(centers[x][y], HTN.BITS.QRCodeLib.Util.Color_Fields.BLUE);
                    canvas.drawLine(new Line(tmpPoint, centers[x][y]), HTN.BITS.QRCodeLib.Util.Color_Fields.LIGHTBLUE);
