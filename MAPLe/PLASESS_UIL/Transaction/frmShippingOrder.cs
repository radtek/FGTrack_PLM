...
namespace HTN.BITS.UIL.PLASESS.Transaction
{
    public partial class frmShippingOrder : BaseChildForm
    {
            ...
            try
            {
                ...
                switch (fState)
                {
                    case eFormState.Add:
                        ...
                        this.btnUnLockSO.Visible = false;
                        ...
                        if (this.xtpPalletItem.PageEnabled)
                            this.xtpPalletItem.PageEnabled = false;
                    ...
                    case eFormState.ReadOnly:
                        ...
                        if (!this.xtpPalletItem.PageEnabled)
                            this.xtpPalletItem.PageEnabled = true;
        ...
        public void GetBindingShippingOrder(string soNo)
        {
            try
            {
                ...
                if (shipOrd != null)
                {
                    ...
                    {
                        this.btnUnLockSO.Image = base.Language.GetBitmap("imgLock");
                        this.btnUnLockSO.Text = "Lock";
                        this.btnUnLockSO.Visible = true;
                    }
                    else
                    {
                        this.btnUnLockSO.Visible = false;
                    ...
                    this.bteDOC_NO.EditValue = this.shipOrd.SALES_ORDER_NO;
        ...
        //public void PostShippingOrderToCSV_OLD(string soNo)
        //{
        ...
        //            dtCsv.Columns.Add("Customer PO");
        //            dtCsv.Columns.Add("Ship Via");
        //            dtCsv.Columns.Add("Customer ID");
        //            dtCsv.Columns.Add("Accounts Receivable Account");
        //            dtCsv.Columns.Add("Displayed Terms");
        //            dtCsv.Columns.Add("Number of Distributions");
        //            dtCsv.Columns.Add("Sales Representative ID");
        //            dtCsv.Columns.Add("Tax Type");
        //            dtCsv.Columns.Add("Sales Tax ID");
        //            dtCsv.Columns.Add("Sales Tax Agency ID");
        ...
        //                if (counter == 1)
        //                {
        //                    if (row.ItemArray[17].Equals("VAT0"))
        //                    {
        //                        hasVAT = false;
        //                        continue;
        //                    }
        //                    else
        //                    {
        //                        dtCsv.Rows.Add(row.ItemArray[0], row.ItemArray[1], row.ItemArray[2], row.ItemArray[3], row.ItemArray[4], row.ItemArray[5], row.ItemArray[6], row.ItemArray[7], row.ItemArray[8], row.ItemArray[9], row.ItemArray[10], row.ItemArray[11], row.ItemArray[12], row.ItemArray[13], row.ItemArray[14], row.ItemArray[15], row.ItemArray[16], row.ItemArray[17], row.ItemArray[18]);
        //                        continue;
        //                    }
        //                }
        ...
        //                if (hasVAT)
        //                {
        //                    dtCsv.Rows.Add(row.ItemArray[0], row.ItemArray[1], row.ItemArray[2], row.ItemArray[3], row.ItemArray[4], row.ItemArray[5], row.ItemArray[6], row.ItemArray[7], row.ItemArray[8], row.ItemArray[9], row.ItemArray[10], row.ItemArray[11], row.ItemArray[12], row.ItemArray[13], row.ItemArray[14], row.ItemArray[15], row.ItemArray[16], row.ItemArray[17], string.Empty);
        //                }
        //                else
        //                {
        //                    dtCsv.Rows.Add(row.ItemArray[0], row.ItemArray[1], row.ItemArray[2], row.ItemArray[3], row.ItemArray[4], row.ItemArray[5], row.ItemArray[6], row.ItemArray[7], row.ItemArray[8], row.ItemArray[9], row.ItemArray[10], row.ItemArray[11], row.ItemArray[12], row.ItemArray[13], row.ItemArray[14], row.ItemArray[15], row.ItemArray[16], string.Empty, string.Empty);
        //                }
        //            }
        ...
        //                //using (CsvWriter writer = new CsvWriter() { Request_Header = true })
        //                //{
        //                //    writer.WriteCsv(dtCsv, filename); //string.Format(@"C:\temp\{0}.CSV", soNo)
        //                //}

        //                using (CsvWriter writer = new CsvWriter() { Request_Header = true, Spliter = "," })
        //                {
        //                    writer.WriteCsv(dtCsv, filename);
        //                }

        //                //string destination = string.Format("{0}\\{1}", ConfigurationManager.AppSettings["HistoryCSVPath"], soNo + ".CSV");
        ...
        public void PostShippingOrderToCSV(string soNo)
        {
            try
            {
                ...
                {
                    prodType = soBll.GetSoProductionType(soNo);
                }



                string fileName = string.Format("{2}-SALESINVOICE{0}-{1:yyyyMMddHHmm}.CSV", soNo, DateTime.Now, prodType);
                ...
                using (SaveFileDialog fdlg = new SaveFileDialog { Title = "Save Export CSV File", Filter = "CSV files (*.csv)|*.csv", FilterIndex = 1, RestoreDirectory = true, FileName = fileName })
                {
                    if (fdlg.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        {
                            dtbPosted = inDataBll.PostSalesInvoiceInterface(soNo, ((frmMainMenu)this.ParentForm).UserID, out seqno);
                        }
                        ...
                        if (dtbPosted != null)
                        {
                            ...
                            dtCsv.Columns.Add("Progress Billing Invoice");
                            dtCsv.Columns.Add("Date");
                            dtCsv.Columns.Add("Ship By");
                            dtCsv.Columns.Add("Quote");
                            dtCsv.Columns.Add("Quote #");
                            dtCsv.Columns.Add("Quote Good Thru Date");
                            ...
                            dtCsv.Columns.Add("Accounts Receivable Account");
                            dtCsv.Columns.Add("Sales Tax ID");
                            dtCsv.Columns.Add("Invoice Note");
                            dtCsv.Columns.Add("Note Prints After Line Items");
                            dtCsv.Columns.Add("Statement Note");
                            dtCsv.Columns.Add("Stmt Note Prints Before Ref");
                            dtCsv.Columns.Add("Internal Note");
                            ...
                            dtCsv.Columns.Add("Apply To Sales Order");
                            dtCsv.Columns.Add("Apply to Proposal");
                            dtCsv.Columns.Add("Quantity");
                            dtCsv.Columns.Add("SO/Proposal Number");
                            dtCsv.Columns.Add("Item ID");
                            dtCsv.Columns.Add("Serial Number");
                            dtCsv.Columns.Add("SO/Proposal Distribution");
                            dtCsv.Columns.Add("Description");
                            dtCsv.Columns.Add("G/L Account");
                            dtCsv.Columns.Add("Unit Price");
                            dtCsv.Columns.Add("Tax Type");
                            ...
                            dtCsv.Columns.Add("Voided by Transaction");
                            dtCsv.Columns.Add("Retainage Percent");
                            ...
                            {
                                dtCsv.Rows.Add(
                                    row.ItemArray[0],    //Customer ID
                                    row.ItemArray[1],    //Invoice/CM #
                                    "",                  //Apply to Invoice Number
                                    "FALSE",             //Credit Memo
                                    "FALSE",             //Progress Billing Invoice
                                    row.ItemArray[2],    //Date
                                    row.ItemArray[3],    //Ship By
                                    "FALSE",             //Quote
                                    "",                  //Quote #
                                    "",                  //Quote Good Thru Date
                                    "FALSE",             //Drop Ship
                                    row.ItemArray[4],    //Ship to Name
                                    row.ItemArray[5],    //Ship to Address-Line One
                                    row.ItemArray[6],    //Ship to Address-Line Two
                                    "",                  //Ship to City
                                    "",                  //Ship to State
                                    "",                  //Ship to Zipcode
                                    row.ItemArray[7],    //Ship to Country
                                    row.ItemArray[8],    //Customer PO
                                    "Truck",             //Ship Via
                                    "",                  //Ship Date
                                    row.ItemArray[9],    //Date Due
                                    row.ItemArray[10],   //Discount Amount
                                    row.ItemArray[11],   //Discount Date
                                    row.ItemArray[12],   //Displayed Terms
                                    "",                  //Sales Representative ID
                                    row.ItemArray[13],   //Accounts Receivable Account
                                    row.ItemArray[14],   //Sales Tax ID
                                    "",                  //Invoice Note
                                    "FALSE",             //Note Prints After Line Items
                                    "",                  //Statement Note
                                    "FALSE",             //Stmt Note Prints Before Ref
                                    "",                  //Internal Note
                                    "FALSE",             //Beginning Balance Transaction
                                    row.ItemArray[15],   //Number of Distributions
                                    row.ItemArray[16],   //Invoice/CM Distribution
                                    "",                  //Apply to Invoice Distribution
                                    row.ItemArray[17],   //Apply To Sales Order
                                    "FALSE",             //Apply to Proposal
                                    row.ItemArray[18],   //Quantity
                                    row.ItemArray[19],   //SO/Proposal Number
                                    row.ItemArray[20],   //Item ID
                                    "",                  //Serial Number
                                    row.ItemArray[21],   //SO/Proposal Distribution
                                    row.ItemArray[22],   //Description
                                    row.ItemArray[23],   //G/L Account
                                    row.ItemArray[24],   //Unit Price
                                    row.ItemArray[25],   //Tax Type
                                    "",                  //UPC / SKU
                                    "0",                 //Weight
                                    row.ItemArray[26],   //Amount
                                    row.ItemArray[27],   //U/M ID
                                    row.ItemArray[28],   //U/M No. of Stocking Units
                                    row.ItemArray[29],   //Stocking Quantity
                                    row.ItemArray[24],   //Stocking Unit Price
                                    "",                  //Job ID
                                    row.ItemArray[30],   //Sales Tax Agency ID
                                    row.ItemArray[31],   //Transaction Period
                                    row.ItemArray[32],   //Transaction Number
                            ...
                            using (ShippingOrderBLL shipOrdBll = new ShippingOrderBLL())
                            {
                                shipOrdBll.UpdatePostSO(soNo, fullFileName, ((frmMainMenu)this.ParentForm).UserID);
                            }
        ...
        public void InsertShippingOrder()
        {
            ...
            try
            {
                ...
                if (this.bteDOC_NO.EditValue != null)
                {
                    shippingOrd.SALES_ORDER_NO = (string)this.bteDOC_NO.EditValue;
                }
            ...
            finally
            {
                ...
                this.GetPalletHdr(shippingOrd.SO_NO);
        ...
        public void UpdateShippingOrder()
        {
            ...
            try
            {
                ...
                {
                    shippingOrd.SALES_ORDER_NO = string.Empty;
                }
                shippingOrd.REF_NO = this.txtREF_NO.Text;
                if (this.dtpREF_DATE.EditValue != null)
                {
        ...
        private void InsertPalletHdr()
        {
            ...
            try
            {
                ...
                if (result.Equals("OK"))
                {
                    this.GetPalletHdr(this.txtSO_NO.Text);
        ...
        private void UnlockAlreadyPostSO(string sono)
        {
            string resultMsg = string.Empty;
            ...
            try
            {
                using (ShippingOrderBLL shipOrdBll = new ShippingOrderBLL())
                {
                    resultMsg = shipOrdBll.UnlockAlreadyPostSO(sono, ((frmMainMenu)this.ParentForm).UserID);
                }

                if (resultMsg == "OK")
                {
                    this.btnUnLockSO.Image = base.Language.GetBitmap("imgUnlock");
                    this.btnUnLockSO.Text = "Unlock";
        ...
        public void GenerateSalesOrderDetail(string whID, string partyId, string salesNo)
        {
            try
            {
                base.BeginProcessing("Begin Load data...", "Please Waiting for Loading Data", this);
                ...
                using (ShippingOrderBLL shipOrdBll = new ShippingOrderBLL())
                {
                    this.dtbShippingOrderDtl = shipOrdBll.GenerateSalesOrderDetail(whID, partyId, salesNo);
                }
        ...
        private void frmShippingOrder_LoadCompleted()
        {
            ...
            this.dteToDate.DateTime = DateTime.Now.AddDays(7d);
        ...
        private void btnAddNew_Click(object sender, EventArgs e)
        {
            ...
            this.lueWarehouse.EditValue = "FG";
        ...
        private void btnUnLockSO_Click(object sender, EventArgs e)
        {
            try
            {
                DialogResult isAuthen = DialogResult.None;
                ...
                if (isAuthen == DialogResult.OK)
                {
                    ...
                    if (result == DialogResult.Yes)
                    {
                        this.UnlockAlreadyPostSO(this.txtSO_NO.Text);
                    }
                }
                else
                {
        ...
        {
            DialogResult result;
            using (frmUploadCSV uploadCSV = new frmUploadCSV() { MASTER_FORM = eMaster_Form.FRM_SHIPPING_ORDER, USER_ID = ((frmMainMenu)this.ParentForm).UserID })
            {
        ...
        private void bteDOC_NO_ButtonClick(object sender, ButtonPressedEventArgs e)
        {
            ...
            {
                fDocList.FormCalling = eFormCalling.fShippingOrder;
                fDocList.PARTY_ID = this.btePARTY_ID.Text;

                fDocList.Shipping_GetSalesOrderList();
        ...
        private void btnGenSalesItem_Click(object sender, EventArgs e)
        {
            try
            {
                //check condition
                if (this.lueWarehouse.EditValue == null) return;
                if (string.IsNullOrEmpty(this.bteDOC_NO.Text)) return;

                this.GenerateSalesOrderDetail((string)this.lueWarehouse.EditValue, this.btePARTY_ID.Text, this.bteDOC_NO.Text);
        ...
        private void btnSO_List_Click(object sender, EventArgs e)
        {
            DialogResult result;
            using (frmDocListInfo docListInf = new frmDocListInfo())
            {
                docListInf.Doc_GetSalesOrderList();
