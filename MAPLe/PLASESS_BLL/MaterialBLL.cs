...
namespace HTN.BITS.BLL.PLASESS
{
    public class MaterialBLL : IDisposable
    {
        ...
        public string UploadMaterialMaster(DataTable dt, string locationId, string userid)
        {
            try
            {
                ProcParam param = new ProcParam(14) { ProcedureName = "MASTER_PACK.UPLOAD_MATERIAL_MASTER" };

                var arrMTL_CODE = dt.AsEnumerable().ToList().Select(r => r.Field<string>("Item ID")).ToArray();
                param.AddParamInput(0, "strMTL_CODE", arrMTL_CODE, OracleDbType.Varchar2);

                var arrMTL_NAME = dt.AsEnumerable().ToList().Select(r => r.Field<string>("Item Description")).ToArray();
                param.AddParamInput(1, "strMTL_NAME", arrMTL_NAME, OracleDbType.Varchar2);

                var arrMTL_NAME_REF = dt.AsEnumerable().ToList().Select(r => r.Field<string>("Description for Sales")).ToArray();
                param.AddParamInput(2, "strMTL_NAME_REF", arrMTL_NAME_REF, OracleDbType.Varchar2);
                ...
                var arrINVEN_ACC_NO = dt.AsEnumerable().ToList().Select(r => r.Field<string>("G/L Inventory Account")).ToArray();
                param.AddParamInput(9, "strINVEN_ACC_NO", arrINVEN_ACC_NO, OracleDbType.Varchar2);

                var arrCOS_ACC_NO = dt.AsEnumerable().ToList().Select(r => r.Field<string>("G/L COGS/Salary Acct")).ToArray();
                param.AddParamInput(10, "strCOS_ACC_NO", arrCOS_ACC_NO, OracleDbType.Varchar2);

                param.AddParamInput(11, "strUSER_ID", ArrayOf<object>.Create(dt.Rows.Count, userid), OracleDbType.Varchar2);

                param.AddParamInput(12, "strLOCATION_ID", ArrayOf<object>.Create(dt.Rows.Count, locationId), OracleDbType.Varchar2);
