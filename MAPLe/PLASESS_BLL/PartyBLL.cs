...
namespace HTN.BITS.BLL.PLASESS
{
    public class PartyBLL : IDisposable
    {
        ...
        public string UploadCustomerMaster(DataTable dt, string partyType, string userid)
        {
            try
            {
                ProcParam param = new ProcParam(7) { ProcedureName = "MASTER_PACK.UPLOAD_PARTY_MASTER" };

                var arrCustomerId = dt.AsEnumerable().ToList().Select(r => r.Field<string>("Customer ID")).ToArray();
                param.AddParamInput(0, "strCUSTOMER_ID", arrCustomerId, OracleDbType.Varchar2);

                var arrCustomerName = dt.AsEnumerable().ToList().Select(r => r.Field<string>("Customer Name")).ToArray();
