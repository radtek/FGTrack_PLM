...
namespace HTN.BITS.BLL.PLASESS
{
    public class JobOrderBLL : IDisposable
    {
                ...
                foreach (JobLotPlan jLotPlan in lstJobLotPlan)
                {
                    switch (jLotPlan.FLAG)
                    {
                        ...
                        case 2: //add new 
                            ...
                            param.AddParamInput(5, "strN_SQUARE", jLotPlan.N_SQUARE);
                            param.AddParamInput(6, "strN_TRIANGLE", jLotPlan.N_TRIANGLE);
                            param.AddParamInput(7, "strUSER_ID", userid);
                            param.AddParamOutput(8, "RESULTMSG", OracleDbType.NVarchar2, 255, "OK");
        ...
        public string UpdateJobLotPlaning(string prodType, string productSeq, DataTable dtbJobLotPlan, string userid)
        {
            ...
            try
            {
                ...
                if (updateRows.Length > 0)
                {
                    ...
                    paramUpd = new ProcParam(12) { ProcedureName = "JOB_ORDER_PACK.JOB_LOT_UPD" };
                    ...
                    //N_SQUARE
                    var arrN_SQUARE = (from DataRow row in updateRows
                                       select row["N_SQUARE"]).ToArray();
                    paramUpd.AddParamInput(6, "strN_SQUARE", arrN_SQUARE, OracleDbType.Varchar2);

                    //N_TRIANGLE
                    var arrN_TRIANGLE = (from DataRow row in updateRows
                                         select row["N_TRIANGLE"]).ToArray();
                    paramUpd.AddParamInput(7, "strN_TRIANGLE", arrN_TRIANGLE, OracleDbType.Varchar2);

                    //REMARK
                    paramUpd.AddParamInput(8, "strREMARK", ArrayOf<object>.Create(updateRows.Length, ""), OracleDbType.Varchar2);
                    //REC_STAT
                    paramUpd.AddParamInput(9, "strREC_STAT", ArrayOf<object>.Create(updateRows.Length, "Y"), OracleDbType.Varchar2);


                    //USER_ID
                    paramUpd.AddParamInput(10, "strUSER_ID", ArrayOf<object>.Create(updateRows.Length, userid), OracleDbType.Varchar2);

                    //RESULTMSG
                    paramUpd.AddParamOutput(11, "RESULTMSG", OracleDbType.NVarchar2, 255, "OK", updateRows.Length);
                }
                ...
                if (insertRows.Length > 0)
                {
                    ...
                    paramIns = new ProcParam(10) { ProcedureName = "JOB_ORDER_PACK.JOB_LOT_INS" };
        ...
        public void InsertLotPlaningToPrint(int seqPrint, List<JobLotPlan> lstJobLotPlan)
        {
            //List<ProcParam> lstParam = new List<ProcParam>(lstJobLotPlan.Count);
            //ProcParam param = null;
            try
        ...
        public void InsertLotPlaningToPostCSV(int seqPrint, List<JobLotPlan> lstJobLotPlan)
        {
            //List<ProcParam> lstParam = new List<ProcParam>(lstJobLotPlan.Count);
            //ProcParam param = null;
            try
            {
                ...
                //param 2 - strTR2
                var arrSHIFT_DATE = (from job in lstJobLotPlan
                                     select string.Format("{0:yyMMdd}",job.SHIFT_DATE)).ToArray();

                paramPrint.AddParamInput(2, "strTR2", arrSHIFT_DATE, OracleDbType.Varchar2);

                //param 3 - strTR3
                var arrSHIFT_ID = (from job in lstJobLotPlan
                                   select job.SHIFT_ID).ToArray();
                paramPrint.AddParamInput(3, "strTR3", arrSHIFT_ID, OracleDbType.Varchar2);
        ...
        public string UpdatePostCSV(string jobno, int seq, string userid)
        {
            string resultMsg = string.Empty;
            ...
            try
            {
                ...
                ProcParam param = new ProcParam(4) { ProcedureName = "JOB_ORDER_PACK.JOBORDER_POSTREF_UPD" };

                param.AddParamInput(0, "strJOB_NO", jobno);
                param.AddParamInput(1, "strSEQ_NO ", seq);
                param.AddParamInput(2, "strUSER_ID", userid);
