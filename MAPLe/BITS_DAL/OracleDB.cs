...
namespace HTN.BITS.DAL
{
    public class OracleDB : ODPDataAccess
    {
        ...
        public override int ExecuteNonQuery(ProcParam paramHdr, ProcParam paramDtl1, int arrBindCount1, ProcParam paramDtl2, int arrBindCount2, ProcParam paramDtl3, int arrBindCount3, ProcParam paramDtl4, int arrBindCount4)
        {
            Stopwatch _stopwatch = null;
            OracleCommand cmd = null;
            ...
            try
            {
                ...
                if (cmd != null)
                {
                    ...
                    #region Execute For Detail 4

                    if (paramDtl4 != null && arrBindCount4 > 0)
                    {
                        cmdDtl4 = base._getCommand(paramDtl4);

                        _stopwatch.Reset();

                        _stopwatch.Start();

                        cmdDtl4.ArrayBindCount = arrBindCount4;

                        ret = cmdDtl4.ExecuteNonQuery();

                        _stopwatch.Stop();

                        if (paramDtl4.IndexToCheck != -1)
                        {
                            OracleString[] arrResult = ((OracleString[])paramDtl4.ReturnValue(paramDtl4.IndexToCheck));

                            var resultCheck = from ar in arrResult
                                              where ar.Value != paramDtl4.MessageToCheck
                                              select ar;
